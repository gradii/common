 ...
 *
 * In Node environments without a global `XMLHttpRequest`, callers can pass an implementation
 * (e.g. from the `xmlhttprequest` npm package) via the optional factory.
 *
 * @publicApi
 */
...
function validateXhrCompatibility(req: HttpRequest<any>) {
  ...
  for (const { property, errorCode } of unsupportedOptions) {
    if (req[property]) {
      console.warn(
        `NG0${errorCode}: A \`HttpClient\` request with the \`${property}\` option was sent using XHR, which does not support it. ` +
          `To use the \`${property}\` option, use the Fetch API by passing \`new FetchBackend()\` to \`createHttpClient()\`.`,
      );
    }
  }
}
...
export class HttpXhrBackend implements HttpBackend {
  ...
  constructor(xhrFactory?: XhrFactory | (() => XMLHttpRequest)) {
    if (xhrFactory == null) {
      this.xhrFactory = {
        build: () => new XMLHttpRequest(),
      };
    } else if (typeof xhrFactory === 'function') {
      this.xhrFactory = { build: xhrFactory };
    } else {
  ...
  handle(req: HttpRequest<any>): Observable<HttpEvent<any>> {
    ...
    // HttpClient.jsonp() without installing JSONP support
    if (req.method === 'JSONP') {
      throw new HttpRuntimeError(
        RuntimeErrorCode.MISSING_JSONP_MODULE,
        `Cannot make a JSONP request without JSONP support. JSONP is not implemented in @gradii/http-client because it is browser-specific (it relies on injecting <script> tags into the DOM).`,
      );
    }

    // Validate that the request is compatible with the XHR backend.
    validateXhrCompatibility(req);

    // Check whether this factory has a special function to load an XHR implementation
    // for various non-browser environments.
    ...
    return source.pipe(
      switchMap(() => {
        ...
        return new Observable((observer: Observer<HttpEvent<any>>) => {
          ...
          let onTimeout: (event: ProgressEvent) => void = onError;
