...
describe('HttpClient', () => {
  ...
  describe('makes a basic request', () => {
    ...
    it('should allow flushing requests with a boolean value', (done) => {
      client.get('/test').subscribe((res) => {
    ...
    it('that returns a stream of events', (done) => {
      lastValueFrom(client.get('/test', { observe: 'events' }).pipe(toArray())).then((events) => {
  ...
  describe('makes a POST request', () => {
    ...
    it('with a json body of false', (done) => {
      client
        .post('/test', false, { observe: 'response', responseType: 'text' })
        .subscribe((res) => {
          expect(res.ok).toBeTruthy();
          expect(res.status).toBe(HttpStatusCode.Ok);
          done();
  ...
  describe('makes a DELETE request', () => {
    it('with body', (done) => {
      const body = { data: 'json body' };
      client
        .delete('/test', { observe: 'response', responseType: 'text', body })
        .subscribe((res) => {
          expect(res.ok).toBeTruthy();
  ...
  describe('makes a request for an error response', () => {
    it('with a JSON body', (done) => {
      ...
      backend
        .expectOne('/test')
        .flush({ data: 'hello world' }, { status: HttpStatusCode.InternalServerError, statusText: 'Server error' });
    });
  });
});
